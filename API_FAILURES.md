...
1. **CORS** — The API at `api.loantube.com` does not allow the origin `https://offers.loantube.com`. The browser blocks the response.
2. **Network** — Some users’ networks (mobile carrier, firewall) block or fail requests to `api.loantube.com`.
...
The **API server** (e.g. `api.loantube.com`) must allow the **page origin** (`https://offers.loantube.com`).

- **Page:** `https://offers.loantube.com` (where the app is loaded from)
- **API:** `https://api.loantube.com` (different origin → cross-origin request)
...
- **Your server (e.g. CloudFront + Lambda, or nginx):** forwards to `https://api.loantube.com/api/leads/application-result?tag=...` and returns the response.
